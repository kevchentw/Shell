...
int mysh_kill(pid_t pid) {
    printf(LIGHT_RED"kill: process pgid=%d\n"NONE, pid);
    kill(pid, SIGKILL);
}
...
struct command *mysh_parse_command(char *line) {
    ...
    char **pipeline_args = (char **) malloc(1000 * sizeof(char *));
...
void mysh_print_promt() {
    ...
    printf(WHITE" - ");
    printf(BLUE"[");
    printf(YELLOW"%d-%d-%d %d:%d:%d", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
    printf(BLUE"]\n");
    printf(LIGHT_BLUE"└─[");
    printf(WHITE"mysh");
